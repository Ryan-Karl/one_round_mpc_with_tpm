...
int accept_and_send(SOCKET & ListenSocket, ifstream & if){
	...
	string fileData;
	while(getline(if, fileData)){
		char * msg[fileData.size()+1];
...
int main(int argc, char ** argv) {
	...
	if(atoi(argv[2]) < MIN_PARTIES){
		cout << "Not enough parties: " << atoi(argv[2]) << endl;
		return 0;
	}
	...
	for(unsigned int i = 0; i < num_parties; i++){
		ifstream if(argv[1]);
		if(!if.good()){
			cout << "Unspecified error opening file " << argv[1] << endl;
			return 0;
		}
		if(accept_and_send(ListenSocket, if) < 0){
			return 1;
		}
	}
