...
int main(int argc, char ** argv) {
	...
	if (!c.sendString(keystring.size() + 1, keystring.c_str())) {
		cout << "Sent key, length of " << std::dec << keystring.size() << endl;
	}
	else {
	...
	char * encStr;
	unsigned int encLen;
	if (!c.recvString(encLen, &encStr)) {
		cout << "Got string" << endl;
	}
	else {
		cout << "Failed receiving string (client)" << endl;
	}
	...
	assert(encVec.size() == encLen);
	vector<BYTE> pad = { 1,2,3,4,5 };
	vector<BYTE> decVec = myTPM.c_RSA_decrypt(encVec, encVec.size(), pad);
